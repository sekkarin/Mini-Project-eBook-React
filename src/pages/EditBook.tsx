...
function EditBook() {
  ...
  const getBook = async () => {
    try {
      ...
      setBookInfo(data);
      const { title, author, authorWebsite, publisher, ISBN, category } = data;
  ...
  const onSubmit = async () => {
    ...
    {
      bookFile && formData.append("bookFile", bookFile);
    }
    try {
      const { data } = await api.put("/books/" + bookInfo?._id, formData, {
    ...
    <Wrapper>
      ...
      <div className="w-[100%] flex justify-center mt-5">
        <div className="p-5 w-[500px]">
          <div className="text-[25px] font-[300]">เเก้ไขข้อมูลหนังสือ</div>
          ...
          <div className="mt-5">
            <div className="w-[100%] flex justify-center items-center flex-col">
              <div className="mb-3 text-sm">รูปปัจจุบัน</div>
              <img src={bookInfo?.coverImage} className="w-[100px]"></img>
            </div>
            {/* <Button
              component="label"
              role={undefined}
